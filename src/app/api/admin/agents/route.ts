...
import { getAgentSummariesForAdmin } from "@/modules/agents/agent.service";
...
export async function GET(request: NextRequest) {
  try {
    ...
    const agents = await getAgentSummariesForAdmin();

    return withRateLimitHeaders(NextResponse.json({ agents }), limited.headers);
  } catch (error) {
    captureServerError(error, { route: "/api/admin/agents" });
    return NextResponse.json(
      { message: error instanceof Error ? error.message : "Could not load agents." },
